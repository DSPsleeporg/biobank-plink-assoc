...
\node[block,right of = DA_G_map,fill=blue!20](DP_G_mformat){Genotype map ASCII file parse};
\node[block,right of = aux_DA_up,fill=blue!20](DP_G_format){Genotype ASCII file conversion};
...
\path(DP_G_format)edge[line]node(DP_G2C){proxy \texttt{ped}}(DP_C);
...
\path(DP_G_mformat)edge[line,color=blue]node(DP_G_m2r){genotype map}(DP_G_format);
%Titles: 
