...
// #####
#ifdef _MSC_VER
#define ALIGN16_F __declspec(align(16))
#define ALIGN16_R
#else
#define ALIGN16_F
#define ALIGN16_R __attribute__((aligned(16)))
#endif
#ifdef __LP64__
...
const uint32_t ALIGN16_F float_exp_lookup_int[] ALIGN16_R = {
...
static inline __m128 fmath_exp_ps(__m128 xx) {
  ...
  __m128i limit = _mm_castps_si128(_mm_and_ps(xx, _mm_castsi128_ps(mask7ff)));
  int32_t over = _mm_movemask_epi8(_mm_cmpgt_epi32(limit, max_x));
  if (over) {
    xx = _mm_min_ps(xx, _mm_castsi128_ps(max_x));
    xx = _mm_max_ps(xx, _mm_castsi128_ps(min_x));
  }
  __m128i rr = _mm_cvtps_epi32(_mm_mul_ps(xx, _mm_castsi128_ps(const_aa)));
  __m128 tt = _mm_sub_ps(xx, _mm_mul_ps(_mm_cvtepi32_ps(rr), _mm_castsi128_ps(const_bb)));
  tt = _mm_add_ps(tt, _mm_castsi128_ps(f1));
  ...
  uint32_t v1 = _mm_extract_epi16(((v4)), ((int32_t)(2)));
  uint32_t v2 = _mm_extract_epi16(((v4)), ((int32_t)(4)));
  uint32_t v3 = _mm_extract_epi16(((v4)), ((int32_t)(6)));
